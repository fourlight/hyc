...
> * 增加了分数计算的 __整除__
> * 分数可以和整数 __直接运算__，不需要再用`covert()`函数转换，且分数 __位置随意__，如下
> > 3.1.0版本
> >```python
> >from hyc.fraction import *
> >from hyc import covert
> >
> ># 表示2分之1加5
> >a = Fraction(2,1)
> >b = covert(5, Fraction)
...
> * 增加了分数计算的 __开方计算__
> * 增加了`fraction`模块的`covert()`函数，可让整数和小数直接化为`Fraction`类型， __直接和分数进行运算__
...
`Fraction`类是`fraction`模块中最主要的部分，它会创建一个分数对象，如`a=Fraction(5, 2)`就可以创建`五分之二`这个分数；__也支持带分数__，比如`a=Fraction(6, 1, num=5)`
